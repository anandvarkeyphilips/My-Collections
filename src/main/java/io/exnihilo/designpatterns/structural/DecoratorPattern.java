package io.exnihilo.designpatterns.structural;// Java program to demonstrate Decorator
// pattern 

import lombok.Data;

// Abstract Pizza class (All classes extend from this)
@Data
abstract class Pizza {
    // it is an abstract pizza
    String description = "Unkknown Pizza";

    public abstract int getCost();
}

/*The decorator class : It extends Pizza to be interchangeable with it
 toppings decorator can  also be implemented as an interface */
abstract class ToppingsDecorator extends Pizza {
    public abstract String getDescription();
}

// Concrete pizza classes 
class FarmHouse extends Pizza {
    public FarmHouse() {
        description = "FarmHouse";
    }

    public int getCost() {
        return 200;
    }
}

class Margherita extends Pizza {
    public Margherita() {
        description = "Margherita";
    }

    public int getCost() {
        return 100;
    }
}

// Concrete toppings classes 
class Barbeque extends ToppingsDecorator {
    Pizza pizza;

    public Barbeque(Pizza pizza) {
        this.pizza = pizza;
    }

    public String getDescription() {
        return pizza.getDescription() + ", Barbeque ";
    }

    public int getCost() {
        return 90 + pizza.getCost();
    }
}

class Paneer extends ToppingsDecorator {
    Pizza pizza;

    public Paneer(Pizza pizza) {
        this.pizza = pizza;
    }

    public String getDescription() {
        return pizza.getDescription() + ", Paneer ";
    }

    public int getCost() {
        return 70 + pizza.getCost();
    }
}

// Driver class and method
public class DecoratorPattern {
    public static void main(String args[]) {
        // create new margherita pizza
        Pizza pizza = new Margherita();
        System.out.println(pizza.getDescription() + " Cost :" + pizza.getCost());

        // create new FarmHouse pizza
        Pizza pizza2 = new FarmHouse();
        // decorate it with fresh tomato & paneer topping
        pizza2 = new Barbeque(pizza2);
        pizza2 = new Paneer(pizza2);
        System.out.println(pizza2.getDescription() + " Cost :" + pizza2.getCost());
    }
} 
